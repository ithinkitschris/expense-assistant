...
import { Text, View, FlatList, Pressable, TextInput, Alert, ScrollView, Animated, useColorScheme, Modal, KeyboardAvoidingView, Platform, Dimensions } from 'react-native';
...
import ExpenseCard, { ExpenseCardWithDate } from './components/ExpenseCard';
...
export default function App() {
  ...
  // Tab state
  const [activeTab, setActiveTab] = useState('expenses'); // 'expenses' or 'grocery'
  ...
  // Add grocery item modal state
  const [isAddGroceryModalVisible, setIsAddGroceryModalVisible] = useState(false);
  const [addGroceryItemName, setAddGroceryItemName] = useState('');
  ...
  const addGroceryItem = async () => {
    // Validate form fields
    if (!addGroceryItemName.trim()) {
      Alert.alert('Error', 'Please enter a grocery item name');
      return;
    }

    try {
      setIsLoading(true);
      await expenseAPI.addGroceryItemDirectly(addGroceryItemName.trim());
      
      // Close modal and clear form
      setIsAddGroceryModalVisible(false);
      setAddGroceryItemName('');
      
      // Reload grocery items to show the new one
      await loadAllGroceryItems();
      
      Alert.alert('Success', 'Grocery item added to pantry!');
    } catch (error) {
      Alert.alert('Error', `Failed to add grocery item: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  // #region RENDER FUNCTIONS
            ...
            >
              ...
              >
                <SymbolView
                  ...
                  size={34}
                  type="monochrome"
                  tintColor={activeTab === 'grocery' ? currentTheme.appleBlue : currentTheme.text}
                  fallback={null}
                />
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                  <Text style={[
    ...
    const renderAddGroceryModal = () => (
      <Modal
        visible={isAddGroceryModalVisible}
        animationType="slide"
        presentationStyle="formSheet"
        onRequestClose={() => setIsAddGroceryModalVisible(false)}
      >
        <KeyboardAvoidingView 
          style={styles.modalContainer}
          behavior={Platform.OS === 'ios' ? 'padding' : 'height'}
        >
          ...
          <View style={styles.modalHeader}>
            <Pressable onPress={() => setIsAddGroceryModalVisible(false)}>
              <Text style={styles.modalCancelButton}>Cancel</Text>
            </Pressable>
            <Text style={styles.modalTitle}>Add Grocery Item</Text>
            <Pressable onPress={addGroceryItem} disabled={isLoading}>
              <Text style={[styles.modalAddButton, isLoading && styles.modalAddButtonDisabled]}>
          ...
          >
            <View style={styles.modalField}>
              ...
              <TextInput
                style={styles.modalInput}
                value={addGroceryItemName}
                onChangeText={setAddGroceryItemName}
      ...
      <View style={styles.addButtonContainer}>
        <Pressable
          ...
          onPress={() => {
            Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Medium);
            if (activeTab === 'grocery') {
              setIsAddGroceryModalVisible(true);
            } else {
