...
export default function App() {
  ...
  const exportDatabase = async () => {
    try {
      ...
      // Create a copy in the cache directory with a timestamp
      const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
      const exportPath = `${FileSystem.cacheDirectory}expenses_backup_${timestamp}.db`;
      ...
      await Sharing.shareAsync(exportPath, {
        ...
        dialogTitle: 'Save Expense Database',
        UTI: 'public.database'
      });
      ...
      Alert.alert('Error', `Failed to export database: ${error instanceof Error ? error.message : 'Unknown error'}`);
    } finally {
      setIsLoading(false);
    }
  };
